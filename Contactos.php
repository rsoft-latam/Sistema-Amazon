...
$consulta=$cn->prepare("SELECT cuenta.correo_electronico,cuenta.foto,persona.nombre,persona.paterno,persona.materno 
FROM cuenta,persona 
WHERE cuenta.codigo_cuenta IN (SELECT codigo_cuenta FROM estado WHERE nombre='Activo')
AND cuenta.codigo_persona=persona.codigo_persona");
$consulta->execute();
